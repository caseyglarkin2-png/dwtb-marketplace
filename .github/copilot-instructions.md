...
Browser → dwtb.dev (Vercel)
            ...
            └─ API Route Handlers (BFF layer)
                       │
                       └─ src/lib/clawd.ts ──→ Railway API
                                                (clawd-control-plane-production.up.railway.app)
...
**POST /api/marketplace/bid**
Body: `{ company, name, email, title, bid_amount, message?, agreement_accepted, consent_hash }`
...
### Route → Clawd Endpoint Mapping (CONFIRMED WIRED)

| Next.js Route | Clawd Endpoint | Notes |
|---|---|---|
| `POST /api/bids` | `POST /api/marketplace/bid` | Creates bid, returns `bid_id` |
| `GET /api/slots` | `GET /api/slots/status` | Maps `available` → `remaining_slots` |
| `GET /api/stats` | `GET /api/dwtb-pipeline` | Maps pipeline stats to public metrics |
| `GET /api/bids/status` | `GET /api/marketplace/bids` (list) | Finds bid by id, verifies email |
| `GET /api/admin/bids` | `GET /api/marketplace/bids` | Admin list |
| `PATCH /api/admin/bids/[id]/status` | `PATCH /api/marketplace/bid/{id}` | Admin status transition |
| `GET /api/admin/stats` | `GET /api/intake/leads/stats` | Lead counts for admin dashboard |

All Clawd calls go through `clawdFetch()` in `src/lib/clawd.ts`. Never call Railway directly from a route.
...
- **Zod schemas**: API response shapes are in `src/lib/api-types.ts` — validate with `safeParse` before using
- **Caching**: `src/lib/cache.ts` provides in-memory TTL cache for Clawd responses (30s default)
- **PDFs**: `src/lib/pdf.ts` generates contract + payment instruction PDFs on acceptance
